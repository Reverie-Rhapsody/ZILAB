...
#base_power_drive = 5 #Q8
#base_power_drive = -15 #Q1
#base_power_drive = -5 #Q1
#base_power_drive = 10 #Q1
#base_power_drive = 18 #C9
#base_power_drive = 10#C7
#base_power_drive = 15#C6
#base_power_drive = 16#C6
base_power_drive = 16#C3

result_length = 25  #UHFQA和HDAWG 此处相同
num_averages_uhfqa = 1024 # 2^N
num_averages_hdawg = 1 # 2^N
time_origin_sec = 200000e-9 #UHFQA和HDAWG 此处相同
period_wait_sec = 1000e-6

#drive_wave_width_sec = 114.03e-9 #length of gauss pulse 单位：s
#drive_pulse_width_sec = 38.01e-9 #standard deviation of gauss pulse 单位：s
...
#drive_wave_width_sec = 19.20e-9 #length of gauss pulse 单位：s C6
#drive_pulse_width_sec = 6.4e-9 #standard deviation of gauss pulse 单位：s C6
#drive_wave_width_sec = 83.59e-9 #length of gauss pulse 单位：s C3
#drive_pulse_width_sec = 27.86e-9 #standard deviation of gauss pulse 单位：s C3
...
drive_wave_width_sec = 25.51e-9 #length of gauss pulse 单位：s C8
drive_pulse_width_sec = 8.503e-9 #standard deviation of gauss pulse 单位：s C8

square_wave_sec_dc = 4e-6

#drive_wave_width_sec = 29.50e-9 #length of gauss pulse 单位：s
#drive_pulse_width_sec = 9.83e-9 #standard deviation of gauss pulse 单位：s
#time_increment = 75 #测T1每次增加的时间长度，为1.8GHz采样率下设置点数对应的时间长度
time_increment = 1 #测T1每次增加的时间长度，为1.8GHz采样率下设置点数对应的时间长度 C4
...
"""设置直流源"""
# DC_1.set_value(ch=1, offset=0.0)
# DC_1.turn_on(ch=1)
# DC_1.get_state(ch=1)
# time.sleep(0.2)
...
#hdawg.turnoff_modulation() #关掉调制
hdawg.set_osc(osc_freq_drive)
hdawg.awg_builder_T1()
hdawg.daq.sync()
hdawg.awg_open()
...
print('------')
print(max(np.array(data)))
print('------')
print(min(np.array(data)))

"""MongoDB存储数据"""
result = [{'time_x':(np.linspace(0, hdawg.result_length-1, hdawg.result_length)).tolist()},{'T1_real':data_ch_1.tolist()},{'T1_imag':data_ch_2.tolist()},{'time_increment':hdawg.time_increment}]
database_host = 'mongodb://localhost:27017/'
database = mongodb(database_host)
database.save_T1(result) 
...
hdawg.awg_builder_T1() #变gauss pulse长度
